<script lang="ts">
  ...
  const circleData = [
    ...
    { major: 'F♯/G♭', minor: 'D♯m/E♭m', sharps: 6, flats: 6, angle: 180, keySignature: '♯♯♯♯♯♯/♭♭♭♭♭♭' },
  ...
  function handleKeyClick(key: string, mode: 'major' | 'minor') {
    ...
    const rootNote = mode === 'major' ? key.split('/')[0].replace('♯', '#').replace('♭', 'b') : 
  ...
  function getKeySignature(key: string, mode: 'major' | 'minor'): string {
    const keyData = circleData.find(data => 
      (mode === 'major' && data.major === key) || 
      (mode === 'minor' && data.minor === key)
    );
    
    if (!keyData) return '';
    
    // List of specific enharmonic keys that should show both signatures on separate lines
    const specificEnharmonicKeys = [
      'A♯m/B♭m', 'D♯m/E♭m', 'G♯m/A♭m', 'C♯/D♭', 'F♯/G♭', 'B/C♭'
    ];
...
<svelte:head>
  ...
  <meta name="description" content="Learn the Circle of Fifths - the ultimate tool for understanding key signatures, chord progressions, and music theory relationships." />
</svelte:head>
  ...
  <header class="header-section">
    <div class="header-content">
      ...
      <p class="page-description">
        Master key signatures, chord relationships, and scales with this interactive Circle of Fifths
      </p>
    </div>
  </header>
      ...
      <div class="circle-card">
        <svg viewBox="0 0 500 500" class="circle-svg">
          ...
          {#each circleData as keyData, index}
            ...
            {@const segmentPath = createSegmentPath(startAngle, endAngle, minorRadius + 10, outerRadius)}
          ...
          {#each circleData as keyData, index}
            {@const startAngle = keyData.angle - 15}
            {@const endAngle = keyData.angle + 15}
            {@const segmentPath = createSegmentPath(startAngle, endAngle, innerRadius, minorRadius + 10)}
          ...
          />
            ...
            <g class="direction-arrows">
              ...
              <path d="M 230 200 A 40 40 0 0 0 200 230" stroke="#666" stroke-width="3" fill="none" marker-end="url(#arrowhead)"/>
              <text x="180" y="270" class="direction-text">Up by 4th</text>
              
              <!-- Up by 5th (clockwise, right side) -->
              <path d="M 270 200 A 40 40 0 0 1 300 230" stroke="#666" stroke-width="3" fill="none" marker-end="url(#arrowhead)"/>
              <text x="263" y="270" class="direction-text">Up by 5th</text>
            </g>
          
          <!-- Arrow marker -->
          <defs>
            <marker id="arrowhead" markerWidth="10" markerHeight="10" refX="0" refY="5" orient="auto" markerUnits="strokeWidth">
    ...
    {#if selectedKey}
      <div class="key-info-panel">
        <div class="glass-card info-card">
          <h3 class="info-title">
            {selectedKey} {selectedMode === 'major' ? 'Major' : 'Minor'}
          </h3>
